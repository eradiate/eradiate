...
class DatasetRegressionFixture:
    ...
    def check(
        ...
        def dump_aux_fn(filename: Path) -> list[str]:
            return self._plot_noref(result, tests, basename) if bootstrapping else []
    ...
    def _plot_noref(
        self, result: xr.Dataset, tests: list[RegressionTest], basename: str
    ) -> list[str]:
        """
        Chart a result that has just been archived as a reference candidate,
        one chart per tested variable.
        """
        if not self.plot:
            return []
        ...
        # `plot_noref()` depends on nothing but the tested variable, so criteria
        # sharing one would draw the same chart twice
        for variable, test in {test.variable: test for test in tests}.items():
            fig, _ = test.plot_noref(result)
            try:
                paths.append(str(self._emit(fig, f"{basename}-{variable}.png")))
            finally:
                plt.close(fig)
