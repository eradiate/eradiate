...
class ParticlePhaseFunction(PhaseFunction):
    ...
    particle_properties: ParticleProperties = documented(
        attrs.field(
            kw_only=True,
            validator=attrs.validators.instance_of(ParticleProperties),
    ...
    def _eval_impl(self, w: pint.Quantity) -> tuple[np.ndarray, np.ndarray]:
        ...
        phase : ndarray
            Evaluated phase function as a 2D array, shape ``(n_phamat, n_mu)``.
        """
        return self.particle_properties.eval_phase(w=w)

    @singledispatchmethod
    def eval_pmom(self, si: SpectralIndex) -> np.ndarray:
        """
        Evaluate the Legendre moments of the (1,1) phase matrix element at a
        ...
        -------
        pmom : ndarray
            Legendre moments for the (1,1) phase matrix element, shape
            ``(nleg,)``.
    ...
    def _(self, si) -> np.ndarray:
        return self._eval_pmom_impl(si.w)
    ...
    def _eval_pmom_impl(self, w: pint.Quantity) -> np.ndarray:
        ...
        -------
        values : ndarray
            Legendre moment array, shape ``(nleg,)``.
        """
        values, _ = self.particle_properties.eval_pmom(w=w, clip=False)
        result = values[:, 0]  # (imom,); squeeze scalar-w axis
