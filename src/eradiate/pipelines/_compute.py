...
class ApplySpectralResponseFunction(PipelineStep):
    ...
    Notes
    -----
    The processed dataset is expected to have a ``w`` coordinate, as well as
    a ``wbounds`` data variable (in CKD mode) containing the lower and upper
    spectral bounds.
    If not, :meth:`transform` will raise an exception.
    """
