...
Multi distant radiance meter goniometer variant (:monosp:`mdistant_gonio`)
-------------------------------------------------
...
In contrast to the :monosp:`mdistant` sensor, this one alwayssamples the target
...
class MultiDistantSensorGonio final : public Sensor<Float, Spectrum> {
public:
    ...
    MultiDistantSensorGonio(const Properties &props) : Base(props) {
    ...
    std::pair<Ray3f, Spectrum> sample_ray(Float time, Float wavelength_sample,
        ...
        if (m_target_type == RayTargetType::Point) {
            ...
            Vector3f perp_offset =
                trafo.transform_affine(Vector3f{ ps.p.x(), ps.p.y(), ps.p.z() });
            ray.o = perp_offset - ray.d * m_ray_offset;
    ...
    std::string to_string() const override {
        ...
        oss << "MultiDistantSensorGonio[" << std::endl
...
MI_IMPLEMENT_CLASS_VARIANT(MultiDistantSensorGonio, Sensor)
MI_EXPORT_PLUGIN(MultiDistantSensorGonio, "MultiDistantSensorGonio")
