...
The statistical criteria themselves are tested in ``test_regression.py``.
...
and are covered upstream. These tests check Eradiate's own behaviour:
missing-reference guard, separation between the reference read and write
directories references are read, archive location.

The test code is complicated by Robot Framework reporting and would likely
simplify if the reporting system would change.
"""
...
# Import the exception type raised by `pytest.fail()` directly from its private
# home rather than through the `pytest.fail.Exception` alias:
# pytest-robotframework wraps `pytest.fail` to log it as a Robot keyword, and
# the wrapper does not carry the alias over (otherwise makes tests fail when
# the pytest-robotframework plugin is loaded).
from _pytest.outcomes import Failed

from eradiate.test_tools.fixtures._regression import DatasetRegressionFixture
from eradiate.test_tools.regression import RegressionTestFailure, RMSETest, ZTest
...
class TestMissingReference:
    ...
    def test_without_flag(self, make_fixture, write_dir, artefact_dir):
        ...
        with pytest.raises(Failed, match="--force-regen"):
    ...
    def test_with_force_regen(self, make_fixture, write_dir):
        ...
        with pytest.raises(Failed, match="File not found"):
    ...
    def test_bootstrap_chart(self, make_fixture, artefact_dir):
        ...
        # before promotion. Charts are named after the variable they show, as
        ...
        assert (artefact_dir / "bootstrap-brf.png").is_file()
...
class TestComparison:
    ...
    def test_reads_and_writes_are_separate(self, make_fixture, read_dir, write_dir):
        ...
        with pytest.raises(Failed, match="Files differ"):
            fixture.check(make_dataset(3.0), RMSETest(0.25, variable="brf"), "case")
...
class TestMultipleTests:
    ...
    def test_bootstrap_charts_every_variable(self, make_fixture, artefact_dir):
        # a bootstrap must show every variable it archives, otherwise the
        # candidate cannot be vetted before promotion
        fixture = make_fixture(plot=True, force_regen=True)
        ...
        assert (artefact_dir / "bootstrap-brf.png").is_file()
        assert (artefact_dir / "bootstrap-hdrf.png").is_file()

    def test_bootstrap_charts_shared_variable_once(self, make_fixture, artefact_dir):
        # criteria sharing a variable draw the same reference candidate chart:
        # emit it once
        fixture = make_fixture(plot=True, force_regen=True)
        ...
        with pytest.raises(Failed):
            fixture.check(
                make_dataset(),
                [RMSETest(0.25, variable="brf"), ZTest(0.05, variable="brf")],
                "bootstrap",
            )

        assert list(artefact_dir.glob("*.png")) == [artefact_dir / "bootstrap-brf.png"]
