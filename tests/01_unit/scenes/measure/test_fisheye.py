...
from eradiate.scenes.measure._fisheye import _convert_lens_coefficients
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "projection_model": "polynomial",
            "lens_coefficients": [0.7195, -0.0637, -0.005],
        },
        {
...
@pytest.mark.parametrize(
    "tested, expected",
    [
        ([0.7195, -0.0637], [0.7195, -0.0637]),
        ("0.7195, -0.0637", [0.7195, -0.0637]),
        ("0.7195 -0.0637", [0.7195, -0.0637]),
        (0.7195, [0.7195]),
        ("0.7195, abc", ValueError),
        ([[0.7195, -0.0637]], ValueError),
    ],
    ids=["list", "str_comma", "str_whitespace", "scalar", "str_invalid", "2d"],
)
def test_fisheye_lens_coefficients_converter(tested, expected):
    if isinstance(expected, list):
        np.testing.assert_allclose(_convert_lens_coefficients(tested), expected)
    else:
        with pytest.raises(expected):
            _convert_lens_coefficients(tested)
...
@pytest.mark.parametrize(
    ...
    [
        {"projection_model": "polynomial"},
        {"projection_model": "equisolid", "lens_coefficients": [0.7195, -0.0637]},
        {**CALIBRATION, "film_resolution": (32, 32)},
    ],
    ids=[
        "polynomial_no_coefficients",
        "coefficients_without_polynomial",
        "aspect_ratio_mismatch",
    ],
)
def test_fisheye_construct_invalid(mode_mono, tested):
    with pytest.raises(ValueError):
        FisheyeCameraMeasure(**tested)
...
def test_fisheye_kernel_dict(mode_mono):
    ...
    np.testing.assert_allclose(kdict["center_x"], 3017 / 6000)
    np.testing.assert_allclose(kdict["center_y"], 1989 / 4000)
    np.testing.assert_allclose(kdict["radius"], 1643 / 6000)
...
def test_fisheye_dataset_attrs(mode_mono):
    attrs = FisheyeCameraMeasure(**CALIBRATION).dataset_attrs()
    assert attrs["projection_model"] == "polynomial"
    np.testing.assert_allclose(attrs["lens_coefficients"], [0.7195, -0.0637])
    np.testing.assert_allclose(attrs["center_x"], 3017 / 6000)
    np.testing.assert_allclose(attrs["center_y"], 1989 / 4000)
    np.testing.assert_allclose(attrs["image_circle_radius"], 1643 / 6000)

    assert "lens_coefficients" not in FisheyeCameraMeasure().dataset_attrs()
...
def test_fisheye_valid_mask(mode_mono):
    ...
    np.testing.assert_array_equal(measure.valid_mask, expected)
...
def test_fisheye_full_scene(mode_mono):
    ...
    np.testing.assert_array_equal(result.valid, measure.valid_mask)
    assert result.attrs == measure.dataset_attrs()
