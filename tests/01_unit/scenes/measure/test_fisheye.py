...
@pytest.mark.parametrize(
    ...
    [
        ...
        CALIBRATION,
        {"projection_model": "polynomial", "lens_coefficients": "0.7195, -0.0637"},
        {"projection_model": "polynomial", "lens_coefficients": "0.7195 -0.0637"},
    ],
    ids=[
        "no_args",
...
def test_fisheye_lens_coefficients_input_forms(mode_mono):
    # Coefficients may be given as a sequence, a string or a scalar
    expected = [0.7195, -0.0637]
    for coefficients in [expected, "0.7195, -0.0637", "0.7195 -0.0637"]:
        measure = FisheyeCameraMeasure(
            projection_model="polynomial", lens_coefficients=coefficients
        )
        assert measure.lens_coefficients == expected

    measure = FisheyeCameraMeasure(
        projection_model="polynomial", lens_coefficients=0.7195
    )
    assert measure.lens_coefficients == [0.7195]
...
def test_fisheye_construct_invalid(mode_mono):
    for kwargs in [
        ...
        {  # Coefficients given as an unparseable string
            "projection_model": "polynomial",
            "lens_coefficients": "0.7195, abc",
        },
    ]:
