...
def test_rescale_concentration():
    ...
    factors = compute_scaling_factors(ds=ds, concentration={"O3": new_ozone_amount})
    with pytest.raises(ValueError):
        rescale_concentration(ds=ds, factors=factors, inplace=False)
...
def test_equilibrium_water_vapor_fraction():
    ...
    # values are in [0, 1]
    value = equilibrium_water_vapor_fraction(
...
def test_find_regular_params_gcd():
    with pytest.raises(ValueError):
        _find_regular_params_gcd([1, 2, 3], -1.0)

    with pytest.raises(ValueError):
        _find_regular_params_gcd([1, 2, 3], 0.0)
    ...
    assert _find_regular_params_gcd(mesh, unit_number=1000.0) == (26, 1000)
