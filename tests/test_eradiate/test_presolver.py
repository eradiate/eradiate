...
from eradiate._presolver import PathResolver
...
def test_resolve(presolver):
    ...
    assert os.path.join(base, "tests/test_eradiate/test_presolver.py") == str(
        presolver.resolve("tests/test_eradiate/test_presolver.py")
    )
