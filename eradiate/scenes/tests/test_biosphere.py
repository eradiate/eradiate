...
def test_leaf_cloud_instantiate_parameters(mode_mono):
    ...
    assert np.allclose([max_x, max_y, max_z], expected_size, atol=1.0e-1)
...
def test_leaf_cloud_instantiate_file(mode_mono, tempfile_leaves):
    ...
    cloud = LeafCloud.from_file(tempfile_leaves, 0.5, 0.5)
...
@pytest.mark.slow
def test_floating_sphere_canopy_instantiate(
    mode_mono, tempfile_leaves, tempfile_spheres
):
    canopy_dict = {
        "leaf_cloud": {
            "fname_leaves": tempfile_leaves,
            "fname_positions": tempfile_spheres,
            "leaf_transmittance": 0.5,
            "leaf_reflectance": 0.5,
        },
        "size": [1, 1, 1] * ureg.m,
