...
* ⚠️ {attr}`.Measure.spp` is now a *total* sample budget, distributed across
  spectral loop iterations proportionally to the measure's spectral response
  function (and, in CKD mode, to quadrature weights). It used to be applied in
  full to every iteration. The previous behaviour is restored by setting
  `sample_allocation = "uniform"` in the configuration file, or the
  `ERADIATE_SAMPLE_ALLOCATION` environment variable to `uniform`.
